...
function MarqueeRow({ logos, direction = "left" }: { logos: string[]; direction?: "left" | "right" }) {
  const marqueeItems = [...logos, ...logos, ...logos]

  return (
    <div className="overflow-hidden">
      <div className={`flex w-max items-center ${direction === "right" ? "marquee-right" : "marquee-left"}`}>
  ...
  return (
    <section className="relative overflow-hidden pt-10 pb-16 text-white md:pt-12 md:pb-20 lg:pt-14 lg:pb-24">
      <div className="absolute inset-0 -z-20">
        <Image src="/Logo BG (1).png" alt="Logo background" fill priority className="object-cover object-center" />
      </div>
      ...
      <div className="relative mx-auto max-w-[1500px] px-6 md:px-8">
        <div className="mx-auto max-w-[1080px] text-center">
          <div className="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/70 md:text-xs">
            <span className={raleway.className}>{isZh ? "品牌信赖之选" : "TRUSTED BY BRANDS"}</span>
          </div>

          <h2 className={`${raleway.className} mt-3 text-[42px] uppercase leading-[1] tracking-[-0.03em] md:text-[60px]`}>
            {isZh ? (
              <>深受全球领先组织信赖，<br className="hidden md:block" />打造高表现品牌星群。</>
            ) : (
              <>TRUSTED BY LEADING ORGANIZATIONS WORLDWIDE,<br className="hidden md:block" />BUILDING A CONSTELLATION OF HIGH-PERFORMING BRANDS.</>
            )}
          </h2>
      ...
      <style jsx global>{`
        .marquee-left { animation: marquee-left 95s linear infinite; }
        .marquee-right { animation: marquee-right 95s linear infinite; }
        .marquee-left:hover, .marquee-right:hover { animation-play-state: paused; }
        @keyframes marquee-left { 0% { transform: translate3d(0,0,0);} 100% { transform: translate3d(-33.333%,0,0);} }
        @keyframes marquee-right { 0% { transform: translate3d(-33.333%,0,0);} 100% { transform: translate3d(0,0,0);} }
      `}</style>
    </section>
  )
}
