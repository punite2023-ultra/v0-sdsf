...
const heroImages = ["/HeroImage1.png", "/HeroImage2.png"]
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#17002d] text-white">
      ...
      <div className="relative mx-auto grid min-h-[760px] max-w-[1500px] items-center gap-6 px-6 pb-14 pt-28 md:px-10 lg:grid-cols-[0.86fr_1.14fr] lg:pt-24 xl:min-h-[820px]">
        ...
        {/* RIGHT IMAGE SLIDER - SAME POSITION */}
        <div className="absolute right-[-5vw] top-[-10%] bottom-0 w-[60vw] max-w-none pointer-events-none">
          {heroImages.map((image, index) => (
            <Image
              key={image}
              src={image}
              alt={`Star Digital Solutions hero ${index + 1}`}
              fill
              priority={index === 0}
              className={`object-contain object-bottom scale-[1.45] drop-shadow-[0_50px_120px_rgba(0,0,0,0.5)] transition-all duration-700 ease-out lg:scale-[1.55] 2xl:scale-[1.65] ${
                activeImage === index
